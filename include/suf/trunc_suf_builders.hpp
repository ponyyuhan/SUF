...
inline SUF<uint64_t> build_gapars_suf(int frac_bits, uint64_t r_low) {
  // GapARS (SIGMA-style) uses the same low-bit carry predicate as truncation.
  // The remaining wrap/sign corrections are handled in post-processing using
  // the proven gap condition (compiler-side).
  return build_trunc_pred_suf(frac_bits, r_low, /*include_sign=*/false);
}
