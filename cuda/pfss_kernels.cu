...
// Optimized T-table implementation (Sigma-style), verified by test_cuda_prg
// against OpenSSL on host.
namespace {
...
// Encryption T-tables (Te0..Te3) for AES-128, big-endian u32 state words.
__device__ __constant__ uint32_t kTe0[256] = {
...
__device__ __constant__ uint32_t kTe1[256] = {
...
__device__ __constant__ uint32_t kTe2[256] = {
...
__device__ __constant__ uint32_t kTe3[256] = {
...
__device__ __forceinline__ void aes128_encrypt_block(uint8_t* state, const uint8_t* round_keys) {
  uint32_t s0 = load_be32(state + 0);
  uint32_t s1 = load_be32(state + 4);
  uint32_t s2 = load_be32(state + 8);
  uint32_t s3 = load_be32(state + 12);
...
#pragma unroll
  for (int r = 1; r <= 9; ++r) {
    uint32_t t0 = kTe0[s0 >> 24] ^ kTe1[(s1 >> 16) & 0xff] ^ kTe2[(s2 >> 8) & 0xff] ^ kTe3[s3 & 0xff] ^
                  load_be32(rk + 0);
    uint32_t t1 = kTe0[s1 >> 24] ^ kTe1[(s2 >> 16) & 0xff] ^ kTe2[(s3 >> 8) & 0xff] ^ kTe3[s0 & 0xff] ^
                  load_be32(rk + 4);
    uint32_t t2 = kTe0[s2 >> 24] ^ kTe1[(s3 >> 16) & 0xff] ^ kTe2[(s0 >> 8) & 0xff] ^ kTe3[s1 & 0xff] ^
                  load_be32(rk + 8);
    uint32_t t3 = kTe0[s3 >> 24] ^ kTe1[(s0 >> 16) & 0xff] ^ kTe2[(s1 >> 8) & 0xff] ^ kTe3[s2 & 0xff] ^
                  load_be32(rk + 12);
    s0 = t0; s1 = t1; s2 = t2; s3 = t3;
    rk += 16;
  }

  uint32_t t0 = (uint32_t(kSBox[s0 >> 24]) << 24) |
                (uint32_t(kSBox[(s1 >> 16) & 0xff]) << 16) |
                (uint32_t(kSBox[(s2 >> 8) & 0xff]) << 8) |
                uint32_t(kSBox[s3 & 0xff]);
  uint32_t t1 = (uint32_t(kSBox[s1 >> 24]) << 24) |
                (uint32_t(kSBox[(s2 >> 16) & 0xff]) << 16) |
                (uint32_t(kSBox[(s3 >> 8) & 0xff]) << 8) |
                uint32_t(kSBox[s0 & 0xff]);
  uint32_t t2 = (uint32_t(kSBox[s2 >> 24]) << 24) |
                (uint32_t(kSBox[(s3 >> 16) & 0xff]) << 16) |
                (uint32_t(kSBox[(s0 >> 8) & 0xff]) << 8) |
                uint32_t(kSBox[s1 & 0xff]);
  uint32_t t3 = (uint32_t(kSBox[s3 >> 24]) << 24) |
                (uint32_t(kSBox[(s0 >> 16) & 0xff]) << 16) |
                (uint32_t(kSBox[(s1 >> 8) & 0xff]) << 8) |
                uint32_t(kSBox[s2 & 0xff]);
  ...
  store_be32(state + 0, t0);
  store_be32(state + 4, t1);
  store_be32(state + 8, t2);
  store_be32(state + 12, t3);
}
